...
import { SopasList } from "../../assets/Data/SopasList";
...
const Sopas = () => {
  const [recetas, setRecetas] = useState(SopasList);
  const handleSearch = (value) => {
    if (!value.length) return setRecetas(SopasList);

    const filterData = SopasList.filter((item) =>
